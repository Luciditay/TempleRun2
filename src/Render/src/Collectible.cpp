...
Collectible::Collectible(glm::vec2 position, std::string type, bool flying, Character *character, ScoreCounter *score) : m_scale(0.8, 0.8, 0.8), m_flying(flying), m_type(type), m_hidden(false), m_char(character), m_score(score)
{
    if (m_flying)
    {
...
bool Collectible::isHidden() const
{
    return m_hidden;
}
